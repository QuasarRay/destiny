...
impl CompatRuntime {
    ...
    fn dispatch_park(
        ...
        match name {
            ...
            "AdjustTimes" => {
                self.park.time = adjust_time(
                    self.park.time,
                    value_i64(args.first(), "time_delta")?,
                )
                .ok_or_else(|| CompatError::InvalidRequest("time adjustment overflow".into()))?;
                Ok(Value::Null)
            }
            "SetBallPosition" => {
                let id = value_i64(args.first(), "ball_id")?;
...
mod tests {
    ...
    fn original_basic_setters_match_observable_state() {
        ...
        runtime.dispatch(request(
            "destiny.Ballpark.SetBallMass", "call", Some("park:0"),
            vec![json!(1), json!(3.14)],
        )).expect("mass");
        assert_eq!(runtime.world().get::<DestinyMass>(entity).expect("mass").0, 3.14);

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallRadius", "call", Some("park:0"),
            vec![json!(1), json!(3.14)],
        )).expect("radius");
        assert_eq!(runtime.metadata(1).expect("metadata").radius, 3.14);

        runtime.dispatch(request(
            "destiny.Ballpark.SetMaxSpeed", "call", Some("park:0"),
            vec![json!(1), json!(3.14)],
        )).expect("max speed");
        assert_eq!(runtime.world().get::<MaxLinearSpeed>(entity).expect("max speed").0, 3.14);

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallPosition", "call", Some("park:0"),
            vec![json!(1), json!(1.0), json!(2.0), json!(3.0)],
        )).expect("position");
        assert_eq!(runtime.position(1).expect("position"), DVec3::new(1.0, 2.0, 3.0));

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallVelocity", "call", Some("park:0"),
            vec![json!(1), json!(1.0), json!(2.0), json!(3.0)],
        )).expect("velocity");
        assert_eq!(
            runtime.world().get::<LinearVelocity>(entity).expect("velocity").0,
            DVec3::new(1.0, 2.0, 3.0)
        );

        runtime.dispatch(request(
            "destiny.Ballpark.SetSpeedFraction", "call", Some("park:0"),
            vec![json!(1), json!(0.2)],
        )).expect("speed fraction");
        assert_eq!(runtime.metadata(1).expect("metadata").speed_fraction, 0.2);

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallFree", "call", Some("park:0"),
        ...
        runtime.dispatch(request(
            "destiny.Ballpark.SetBallMassive", "call", Some("park:0"),
            vec![json!(1), json!(false)],
        )).expect("not massive");
        runtime.dispatch(request(
            "destiny.Ballpark.SetBallMassive", "call", Some("park:0"),
        ...
        runtime.dispatch(request(
            "destiny.Ballpark.SetBallGlobal", "call", Some("park:0"),
            vec![json!(1), json!(true)],
        )).expect("global");
        assert!(runtime.metadata(1).expect("metadata").is_global);

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallAgility", "call", Some("park:0"),
            vec![json!(1), json!(3.14)],
        )).expect("agility");
        assert_eq!(runtime.metadata(1).expect("metadata").agility, 3.14);

        runtime.dispatch(request(
            "destiny.Ballpark.SetBallInteractive", "call", Some("park:0"),
            vec![json!(1), json!(false)],
    ...
    fn original_center_and_surface_distance_cases_match() {
        ...
        let center_same = runtime.dispatch(request(
            "destiny.Ballpark.GetCenterDist", "call", Some("park:0"),
            vec![json!(1), json!(2)],
        )).expect("center same");
        let surface_same = runtime.dispatch(request(
            "destiny.Ballpark.GetSurfaceDist", "call", Some("park:0"),
            vec![json!(1), json!(2)],
        )).expect("surface same");
        assert_eq!(center_same, json!(0.0));
        assert_eq!(surface_same, json!(0.0));

        runtime.set_position(2, DVec3::new(100.0, 0.0, 0.0)).expect("axis position");
        assert_eq!(
            runtime.dispatch(request(
                "destiny.Ballpark.GetCenterDist", "call", Some("park:0"),
                vec![json!(1), json!(2)],
        ...
        runtime.set_position(2, DVec3::new(1.0, 2.0, 3.0)).expect("3d position");
        let observed = runtime.dispatch(request(
            "destiny.Ballpark.GetSurfaceDist", "call", Some("park:0"),
            vec![json!(1), json!(2)],
        )).expect("3d surface").as_f64().expect("number");
